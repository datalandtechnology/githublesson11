...
function addtoui(transaction){
	...
	newli.innerHTML = `${transaction.remark} <span>${transaction.amount}</span><span>${transaction.date}</span><button type="button" class="delete-btn" onclick="removetransaction(${transaction.id})">&times;</button>`;
...
// var result = myarrs.reduce(function(total,curvlaue,curidx,arr){
// 		// console.log('this is total = ',total); //0 undefined // if we use 1 peramenter 10 undefined 
	
// 	// console.log('this is curvlaue = ',curvlaue); // 10 to 100 by number // if we use 1 perameter 20 to 100 by number
// 	// console.log('this is curidx = ',curidx); //0 to 9 index number // if we use 1 perameter 1 to 9 index number
// 	// console.log(arr);
